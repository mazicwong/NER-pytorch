...
# for the use of python console in pycharm, with which can set the paht here
import sys
sys.path.append("/home/hzq/tmp/ChineseNER/pytorch/")

# important to know what you had read from the dataset, before doing some change, take several mins to learn about it
...
for epoch in range(EPOCHS):
    ...
    for sentence, tags in zip(x_test,y_test):
        sentence=torch.tensor(sentence, dtype=torch.long).to(device) # revert tensor from np
        score,predict = model(sentence)
        sentence = sentence.cpu().numpy() # return to the numpy format for the use of calcing F-score
